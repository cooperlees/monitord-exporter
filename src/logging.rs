...
#[cfg(feature = "otlp")]
pub fn init(log_filter_level: LevelFilter) -> Option<opentelemetry_sdk::trace::SdkTracerProvider> {
...
pub fn init(log_filter_level: LevelFilter) {
    let fmt = fmt::Layer::default()
        .with_writer(std::io::stderr)
        .with_ansi(stderr().is_terminal())
        .event_format(Glog::default().with_timer(tracing_glog::LocalTime::default()))
...
///   is installed.
#[cfg(feature = "otlp")]
fn otlp_tracer_provider() -> Option<opentelemetry_sdk::trace::SdkTracerProvider> {
